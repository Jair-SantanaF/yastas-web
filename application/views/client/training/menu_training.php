...
<body>
    ...
    <div class="row" style="display: flex; justify-content: space-around; align-items: center; color: white; padding-bottom: 20px;">
        ...
        <div class="col-md-10">
            <p style="font-weight: bold; font-size: large; margin-left: 31%;">Mi aprendizaje</p>
        </div>
    </div>
...
<script>
    ...
    function getTrainings() {
        ...
        $.ajax({
            url: window.base_url + "capacitacion/getCapacitaciones",
            type: "POST",
            dataType: "json",
            data: {
            ...
            success: function(data) {
                var quesionnaries = $("#capacitaciones");
                quesionnaries.empty();
                $.each(data.data, function(i, item) {
                    quesionnaries.append(
                        ...
                        '<div class="text-center to_bottom">' +
                        '<a onclick="selectT(' + item.id + ')" class="btn">Abrir</a>' +
                        '</div>' +
                        '</div> </div> </div>');
                });
            },
