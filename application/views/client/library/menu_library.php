...
<head>
    ...
    <link rel="stylesheet" href="<?php echo base_url() ?>assets/css/client/menu_general.css">
</head>
...
<body>
    ...
    <div class="row" style="display: flex; justify-content: space-around; align-items: center; color: white; padding-bottom: 20px;">
        ...
        <div class="col-md-10">
            <p style="font-weight: bold; font-size: large; margin-left: 31%;">Biblioteca</p>
        </div>
    </div>

    <div class="container">
        <div class="row gx-5" id="elementos" style="margin: 0 auto; max-width: 800px;">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
...
<script>
    ...
    function getTrainings() {
        ...
        $.ajax({
                ...
                $.each(data.data, function(i, item) {
                    ...
                    switch (item.type) {
                        case 'video':
                            ...
                            html += '<div class="text-center to_bottom">' +
                                '<a onclick=selectVideo(' + item.video + ',' + item.id + ') class="btn">Abrir</a>' +
                                '</div>' +
                                '</div> </div> </div>';
                            break;
                        case 'image':
                            ...
                            html += '<div class="text-center to_bottom">' +
                                '<a onclick=selectImage(' + item.image + ',' + item.id + ') class="btn">Abrir</a>' +
                                '</div>' +
                                '</div> </div> </div>';
                            break;
                        case 'documento':
                            ...
                            html += '<div class="text-center to_bottom">' +
                                '<a onclick=selectPdf(' + item.file + ',' + item.id + ') class="btn">Abrir</a>' +
                                '</div>' +
                                '</div> </div> </div>';
                            break;
                    }
            ...
            error: function(data) {
                questionnaires.append("<tr><td colspan='3'>No hay capacitaciones disponibles</td></tr>");
            }
        });
    }
