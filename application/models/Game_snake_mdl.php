<?php
class Game_snake_mdl extends CI_Model
{
    ...
    {
        return $this->db->insert("game_snake_results", $data);
    }
    ...
    public function obtenerMejorPuntuacion($business_id, $user_id)
    {
        ...
        $this->db->query($query);
        $this->db->select("concat(u.name, ' ',u.last_name) as nombre_usuario,concat(u.name, ' ',u.last_name) as nombre,(gr.score - gr.incorrectas) as score, date_format(gr.fecha,'%d %M %Y') as fecha, gr.id_tema, gt.nombre as nombre_tema");
        $this->db->from("game_snake_results as gr");
        $this->db->join("game_snake_temas as gt", "gt.id = gr.id_tema");
    ...
    {
        $this->db->select("concat('" . base_url("assets/img/snake/") . "',image) as image");
        $this->db->from("game_snake_images");
    ...
    public function obtenerTemas($business_id)
    {
        ...
        $this->db->from("game_snake_temas");
        $this->db->where("business_id", $business_id);
        $temas = $this->db->get()->result_array();
        for ($i = 0; $i < count($temas); $i++) {
            $id = $temas[$i]["id"];
            $this->db->select("palabra, tipo");
            $this->db->from("game_snake_palabras");
    ...
    public function obtenerPalabras($business_id)
    {
        ...
        $id = $tema["id"];
        $this->db->select("id,palabra, tipo");
        $this->db->from("game_snake_palabras");
        $this->db->where("id_tema", $id);
        $this->db->order_by("rand()");
        $palabras = ["palabras" => $this->db->get()->result_array()];
    ...
    public function guardar($tema, $palabras, $business_id)
    {
        ...
        $this->db->insert("game_snake_temas", $data);
        $id_tema = $this->db->insert_id();
        for ($i = 0; $i < count($palabras); $i++) {
            $data_ = [];
            $data_["palabra"] = $palabras[$i]["palabra"];
            $data_["tipo"] = $palabras[$i]["tipo"] == 1 ? true : false;
            $data_["id_tema"] = $id_tema;
            $this->db->insert("game_snake_palabras", $data_);
        }
        return true;
    }
    ...
    public function eliminar($id_tema)
    {
        ...
        $this->db->delete("game_snake_temas");

        $this->db->where("id_tema", $id_tema);
        $this->db->delete("game_snake_palabras");
        return true;
    }
    ...
    public function editar($datos)
    {
        ...
        $this->db->update("game_snake_temas");
        ...
        for($i = 0; $i < count($datos["palabras"]); $i++) {
            $data_ = [];
            $data_["palabra"] = $datos["palabras"][$i]["palabra"];
    ...
    {
        $this->db->insert("game_snake_retos", $data);
    ...
    public function agregarOponentes($data, $id_reto)
    {
        for ($i = 0; $i < count($data); $i++) {
            ...
            $this->db->insert("game_snake_retos_users", $data_);
        }
    ...
    public function obtener_retos($pendientes, $user_id)
    {
        ...
        gr.id_tema, g.nombre
        from game_snake_retos as gr
        join game_snake_temas as g on g.id = gr.id_tema
        left join game_snake_results as grr on grr.id_reto = gr.id and grr.user_id = $user_id
        $where";
    ...
    function obtener_resultados($id)
    {
        ...
        from game_snake_retos_users as gu
        left join game_snake_results as gr on gu.id_reto = gr.id_reto and gr.user_id = gu.id_usuario
    ...
    {
        $query = "select palabra from game_snake_palabras where id in ($listado)";
        $palabras =  $this->db->query($query)->result_array();
        $palabras_ = [];
        for ($i = 0; $i < count($palabras); $i++) {
            array_push($palabras_, $palabras[$i]["palabra"]);
        }
        return $palabras_;
    }
}
