...
class Podcast_mdl extends CI_Model
{
    ...
    public function ListPodcast($business_id, $user_id, $es_admin)
    {
        $group = 0;
        if (!$es_admin)
        ...
        $query = "
        select p.id,p.title,p.description,p.preview,p.type,p.audio,
        ...
        and (pu.user_id = ? or pg.group_id in (?))"; //user_id //group_id
