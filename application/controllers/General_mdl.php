...
class General_mdl extends CI_Model
{
    ...
    {
        $this->db->select("aes_decrypt(u.password,'" . KEY_AES . "') as password,u.email, u.business_id");
        ...
        $query = "update user set password = AES_ENCRYPT('" . $password . "','" . KEY_AES . "') where email = '" . $email . "'";
        $this->db->query($query);
    ...
    public function EnviarNotificacionPush($tokens, $mensaje, $titulo, $service_id, $multiple = true, $data = null)
    {
        ...
        if (count($tokens_separados["ios"]) > 0)
            $this->EnviarNotificacionPushIos($tokens_separados["ios"], $mensaje, $titulo, $service_id, $multiple = true, $data = null);
    ...
    public function separar_tokens($tokens)
    {
        ...
        for ($i = 0; $i < count($tokens); $i++) {
            ...
            limit 1
            ";
            $tipo = $this->db->query($query)->result_array()[0]["tipo"];
            if ($tipo == "Android") {
    ...
    public function sendemail($subject, $body, $address, $fromname, $archivo = null)
    {
        ...
        if($archivo != null){
            ...
            $base64 = 'data:'.$archivo["type"]. ';base64,' . base64_encode($data);
    ...
    function writeLog($mensaje, $severidad)
    {
        ...
        $handle = fopen(BASE_PATH . 'application/logs/nup_log' . date('d_m_Y') . '.log', 'a+');
    ...
    function obtenerInsignias($user_id, $business_id)
    {
        ...
        $this->db->select("i.id,i.titulo,i.descripcion,i.business_id,concat('" . $url . "',i.imagen) as imagen, concat('" . $url . "',i.imagen_inactivo) as imagen_inactivo,coalesce(iu.status,0) as estatus, if(cl.id is null,0,1) as permitir_descargar, cl.tipo");
    ...
    function obtener_asesores($id_region)
    {
        ...
        from user where id_region = $id_region and rol_id = 6";
