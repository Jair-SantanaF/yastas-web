...
function ObtenerTablaElementos(record) {
    ...
    tabla_pending = $('#tabla_pending').DataTable({
        ...
        "order": [[0, "asc"]],
        "ajax": {
            url: "../index.php/Groups/LibraryGroups",
            type: 'POST',
            data: function (d) {
        ...
        },
        {
            ...
            render: function (data, type, row) {
                var t = '';
                t = '<div class="pt-2 pb-2">' + row.subcategory != 'null' ? row.subcategory : "" + '</div>';
                return t;
            }
        },
        {
            ...
            render: function (data, type, row) {
                var t = '';
                t = '<div class="pt-2 pb-2">' + row.grupos + '</div>'
                return t;
            }
        },
        {
            ...
            render: function (data, type, row) {
                ...
                if (t != '') {
                    t = '<div class="pt-2 pb-2 d-flex justify-content-center"><a href="' + t + '" target="_blank">Ver...<a><p></div>';
                }
                return t;
            }
        }, {
...
function EliminarElemento(record) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: window.base_url + "Groups/DeleteLibrary",
                type: "POST",
                data: {
                    id: record.id
                },
...
function ObtenerTablaElementosNoAsignados(record) {
    ...
    tabla_elements = $('#tabla_elements').DataTable({
        ...
        "order": [[1, "asc"]],
        "ajax": {
            url: "../index.php/Groups/NoLibraryGroups",
            type: 'POST',
            data: function (d) {
                d.group_id = $('#selected_group_id').val();
            },
...
function AgregarElementos() {
    ...
    $.ajax({
        url: window.base_url + "Groups/SaveLibrary",
        type: "POST",
        data: data,
        cache: false,
        success: function (response) {
