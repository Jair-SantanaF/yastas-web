...
function obtenerUsuarios() {
    $.ajax({
        url: window.base_url + "ResetPassword/obtenerUsuarios",
        type: "POST",
        data: {},
        cache: false,
...
function resetearPassword(id_usuario) {
    if (confirm("La contraseña de este usuario se restablecerá, ¿Está seguro de continuar?"))
        $.ajax({
            url: window.base_url + "ResetPassword/resetear_password",
            type: "POST",
            ...
            success: function (response) {
                ...
                Swal.fire({
                    ...
                    text: 'La contraseña se ha resetado correctamente'
                });
            },
            error: function (error) {
                console.log(error)
...
function generarTablaUsuarios(usuarios) {
    ...
    for (var i = 0; i < usuarios.length; i++) {
        ...
        html += "<td><button class='btn btn-danger' onclick='resetearPassword(" + usuarios[i].id + ")'>Reset</button></td>"
        html += "</tr>"
    }
